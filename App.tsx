...
import { Sun, Moon, Languages, Bell, Search, User as UserIcon, Sparkles, Loader2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // جلب البيانات من الخادم عند بدء التشغيل
  useEffect(() => {
    const initApp = async () => {
      try {
        const remoteUser = await db.getUser();
        if (remoteUser) {
          setUser(remoteUser);
          // جلب كل البيانات لحساب XP
        ...
        console.error("Failed to connect to server database");
      } finally {
  ...
  // تحديث XP ديناميكياً عند التنقل (لضمان الدقة مع قاعدة البيانات)
  useEffect(() => {
  ...
  }, [currentPage, user, isInitialLoading]);
  ...
  if (isInitialLoading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 font-cairo">
        ...
        <h2 className="text-2xl font-black text-slate-800 dark:text-white animate-pulse">جاري الاتصال بخادم قاعدة البيانات...</h2>
        <p className="text-slate-400 font-bold mt-2 uppercase tracking-widest text-xs">Smart Student AI Server</p>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <Auth 
        onLogin={async (u) => {
          const saved = await db.saveUser(u);
          setUser(saved || u);
        }} 
        lang={lang} 
        setLang={setLang} 
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-24 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-12 z-20 transition-all">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 rounded-full border border-emerald-100 dark:border-emerald-800">
