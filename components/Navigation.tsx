...
export const Navigation: React.FC<NavProps> = ({ currentPage, setPage, lang, onLogout }) => {
  ...
  return (
    <aside className={`w-72 bg-white dark:bg-slate-900 h-screen border-${lang === 'ar' ? 'l' : 'r'} border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl z-30 transition-all duration-300 shrink-0`}>
      <div className="p-8 flex items-center gap-4 border-b border-slate-100 dark:border-slate-800">
        <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl">
          <GraduationCap size={28} />
        </div>
        <div>
          <h1 className="text-xl font-black text-slate-900 dark:text-white leading-none">{t.brand_title}</h1>
          <p className="text-[10px] text-indigo-500 font-black uppercase tracking-widest mt-1">Smart Learning</p>
        </div>
      </div>
      ...
      <nav className="flex-1 p-6 space-y-8 overflow-y-auto custom-scrollbar">
        {menuGroups.map((group, gIdx) => (
          <div key={gIdx} className="space-y-2">
            <h3 className={`px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>{group.title}</h3>
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
          <button 
            ...
            className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all ${
              currentPage === 'pricing' ? 'bg-amber-500 text-white shadow-lg' : 'text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20'
            } ${lang === 'ar' ? 'flex-row' : 'flex-row-reverse'}`}
          >
            <Crown size={18} />
      ...
      <div className="p-6 border-t border-slate-100 dark:border-slate-800">
        <button 
          onClick={onLogout}
          className={`w-full flex items-center gap-4 p-4 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl transition-all font-bold text-sm ${lang === 'ar' ? 'flex-row' : 'flex-row-reverse'}`}
        >
          <LogOut size={18} />
